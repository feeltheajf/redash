...
def extract_cached_query_ids(query):
    queries = re.findall(
        r"(?:join|from)\s+cached_query_(\d+)", query, re.IGNORECASE
    )
    return [int(q) for q in queries]
...
def get_query_results(user, query_id, bring_from_cache):
    ...
    if bring_from_cache:
        if query.latest_query_data_id is not None:
            ...
            raise Exception(
                "No cached result available for query {}.".format(query.id)
            )
    else:
...
def create_tables_from_query_ids(
    user, connection, query_ids, cached_query_ids=[]
):
    ...
    try:
        ...
        if not columns:
            raise NoColumnsError(
                u"Error creating table {}: {}".format(table_name, "no columns")
            )
